...
public class LEDStrip extends SubsystemBase {
  ...
  private RGB colorToRGB(LEDColor color) {
    switch (color) {
      ...
      case ORANGE:
        return new RGB(255, 43, 0);
      case YELLOW:
        return new RGB(230, 223, 0);
      case GREEN:
        return new RGB(0, 250, 0);
      case BLUE:
        return new RGB(0, 59, 174);
      case VIOLET:
        return new RGB(255, 0, 255);
      case WHITE:
        return new RGB(221, 222, 223);
      case OFF:
      default:
  ...
  //   if ((millis >= 1*interval && millis < 2*interval) || (millis >= 3*interval && millis <
  // 4*interval)) {
  ...
  public void setBottomColor(LEDColor color) {
    ...
    System.out.println(
        "Changing LED bottom color from "
            + colorToString(lastColorBottom)
            + " to "
            + colorToString(color));
  ...
  public void setTopColor(LEDColor color) {
    ...
    System.out.println(
        "Changing LED top color from "
            + colorToString(lastColorTop)
            + " to "
            + colorToString(color));
  ...
  public AddressableLEDBuffer getTwoColorBuffer(LEDColor bottomColor, LEDColor topColor) {
    ...
    if (bufferCache.containsKey(key)) {
      System.out.println(
          "Cache hit for Buffer Factory for "
